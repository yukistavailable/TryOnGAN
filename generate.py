...
def generate_images_from_outside(
    ...
    if projected_w is not None:
        if z is not None:
            print('warn: z is ignored when using projected-w')
        print(f'Generating images from projected W')
        w = projected_w
    ...
    img = G(z, label, truncation_psi=truncation_psi,
            noise_mode=noise_mode, force_fp32=True)
    img = (
        img.permute(
            0,
            2,
...
@click.option('--trunc', 'truncation_psi', type=float,
              help='Truncation psi', default=1, show_default=True)
@click.option('--class', 'class_idx', type=int,
              help='Class label (unconditional if not specified)')
@click.option('--noise-mode',
              help='Noise mode',
              type=click.Choice(['const',
                                 'random',
                                 'none']),
              default='const',
              show_default=True)
