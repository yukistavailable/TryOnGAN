...
class PoseEncoder(nn.Module):
    def __init__(self, imageSize=256, channel_base=32768):
        ...
        channels = {res: min(channel_base // res, 512)
                    for res in self.block_resolutions}

        self.conv64x64 = nn.Conv2d(17, channels[64], kernel_size=3, padding=1)

        self.conv32x32 = nn.ModuleList(
            [nn.Conv2d(channels[64], channels[32], kernel_size=3, padding=1), nn.MaxPool2d(2)])

        self.conv16x16 = nn.ModuleList(
            [nn.Conv2d(channels[32], channels[16], kernel_size=3, padding=1), nn.MaxPool2d(2)])

        self.conv8x8 = nn.ModuleList(
            [nn.Conv2d(channels[16], channels[8], kernel_size=3, padding=1), nn.MaxPool2d(2)])

        self.conv4x4 = nn.ModuleList(
            [nn.Conv2d(channels[8], channels[4], kernel_size=3, padding=1), nn.MaxPool2d(2)])

        self.conv4x4_2 = nn.Conv2d(
            channels[4],
            channels[4],
            kernel_size=3,
            padding=1)
    ...
    def forward(self, x, only4=False):
        ...
        x4 = torch.div(x4, torch.sqrt(torch.tensor(2, dtype=torch.float32)))
        ret[4] = x4
